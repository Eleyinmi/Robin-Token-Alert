"""
Robin Token Alert — Vercel WSGI entrypoint.

app is a plain WSGI callable with NO top-level third-party imports so Vercel
can import and validate this file before pip install runs. Flask and bot_lib
are imported lazily on the very first request.
"""
...
_flask_app = None  # populated on first request


def _build_app():
    """Build and return the Flask app. Called once, on first WSGI request."""
    ...
    @flask_app.route("/api/scan", methods=["GET", "POST"])
    def scan():
    ...
    @flask_app.route("/api/bot", methods=["GET"])
    def bot_health():
...
def app(environ, start_response):
    """
    Top-level WSGI callable. No third-party imports at module level — safe for
    Vercel to import before pip install. Flask loads on the first real request.
    """
    global _flask_app
